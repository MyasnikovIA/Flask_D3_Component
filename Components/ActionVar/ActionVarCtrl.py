...
class ActionVar(BaseCtrl):
    ...
    def Show(self):
        # Если тип переменной session , тогда элемент не рисуем
        if self.attrs['srctype'] == 'session':
            return ""
        self.sysinfo = []
        self.elProp = "  ".join(f"{k}='{v}'" for k, v in self.attrs.items())
        self.sysinfo.append(f""" <{self.printTag} {self.elProp} ></{self.printTag}> """)
        return f"", [], self.sysinfo,""
